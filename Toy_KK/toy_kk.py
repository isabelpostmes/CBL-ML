...
# Sample the Gaussian in a range of x

# Model for ZLP
def Igaussian(x):
    sigma = 1 # eV (variance)
    A = 1/math.sqrt(2*math.pi) # normalisation
    Igaussian = A*math.exp( -x**2/sigma**2 )
    return Igaussian
...
xmin=-2
xmax=2
gaussian_xspace=np.zeros(N)

x = np.linspace(-N*T/2,N*T/2, N)
xf = np.linspace(0, 1.0/(2.0*T), int(N/2))

for ix in range(N):
    gaussian_xspace[ix]=Igaussian(x[ix])
    
from scipy.fftpack import fft, ifft
gaussian_fourier=np.abs(fft(gaussian_xspace[:N//2]))


gaussian_xspace_inverted=ifft(fft(gaussian_xspace))


plt.clf()
fig = plt.figure(figsize=(5,3.5))
plt.plot(x,gaussian_xspace,linewidth=2.5,color="black",label=r"x-space")
plt.plot(xf,gaussian_fourier,linewidth=2.5,color="blue",ls="dashed",label=r"k-space")
plt.plot(x,gaussian_xspace_inverted,linewidth=2.5,color="red",ls="dotted",label=r"x-space (inverted)")
plt.legend(frameon=True,loc="best",fontsize=14)

plt.tight_layout()
plt.savefig("toy_FT.pdf")
