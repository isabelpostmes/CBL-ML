...
arising in spectra recorded on specimens, specifically on
EELS measurements acquired in different regions
of the WS$_2$ nanostructures presented in Sect.~\ref{sec:tmd}.
%
The resulting ZLP parametrisation will then be applied to isolate the inelastic
contribution in each spectrum.
%
We will use these ZLP-subtracted spectra first to determine the bandgap energy $E_{\rm BG}$
and type from the behaviour of the onset region, and second to identify excitonic
transitions at ultra-low energy losses.

In this section we begin by presenting the training dataset, composed by two sets of spectra recorded
...
%
The resulting subtracted EEL spectra provide the input required to extract the value and type of the bandgap
and to characterise excitonic transitions for different regions of these polytypic 2H/3R WS$_2$ nanostructures.
