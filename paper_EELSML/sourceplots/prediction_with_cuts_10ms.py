...
for name, group in groups_cut:
    ...
    if group['time'].max() == .1 and group['energy'].max() == 2:
        p2=ax.plot(group.x*1000, np.divide(mean_prediction, mean_prediction), label='with data cut',color=rescolors[0],ls="dashed")
...
ax.set_title('$t_{exp}$ = 10 ms, $E_b$ = 200 keV', fontsize = 15)
...
ax.legend([(p1[0],p1a),(p2[0],p2a)],["no data cut","with data cut"], frameon=True,loc=[0.53,0.74],prop={'size':11})
