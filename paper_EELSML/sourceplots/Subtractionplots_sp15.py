...
mean_rep = pd.read_csv('data/Subtracted_spectrum_sp15.csv')
...
amp = 2190
pars = [amp, 1.62, 1.296]
pars_high = [amp, 1.62, (1.296 + 0.3)]
pars_low = [amp, 1.62, (1.296 - 0.7)]
nbins = 493
...
for i in range(1):
    ...
    p1=ax.plot(np.linspace(-.3, 12, nbins), mean_rep['spectrum15'], 'k--',ls="dashdot",lw=2)

    
    p2=ax.plot(np.linspace(-.3, 12, nbins), mean_rep['match15_median'], color=rescolors[1],ls="dashed",lw=2)
    ax.fill_between(np.linspace(-.3, 12, nbins), mean_rep['match15_low'], mean_rep['match15_high'], \
                    color=rescolors[1], alpha=.2)
    p2b=ax.fill(np.NaN,np.NaN,color=rescolors[1],alpha=0.2)
    
    p3 = ax.plot(np.linspace(-.3, 12, nbins), mean_rep['dif15_median'], 'k-',color=rescolors[0],lw=2)
    ax.fill_between(np.linspace(-.3, 12, nbins), mean_rep['dif15_low'], mean_rep['dif15_high'], color=rescolors[0], alpha=.2)
    p3b=ax.fill(np.NaN,np.NaN,color=rescolors[0],alpha=0.2)
    ax.set_yticks([])

    ax.legend([(p1[0]),(p3[0],p3b[0]),(p2[0],p2b[0])],['sp15 (orig)','sp15 (subtr)',"ZLP"],loc='upper right', fontsize = 19)
    ...
    p1=axins.plot(np.linspace(-.3, 12, nbins), mean_rep['dif15_median'], 'k-', alpha=.8, label='sp15 (subtr)',color=rescolors[0])
    axins.fill_between(np.linspace(-.3, 12, nbins), mean_rep['dif15_low'], \
                       mean_rep['dif15_high'], color=rescolors[0],alpha=0.2,lw=2)
