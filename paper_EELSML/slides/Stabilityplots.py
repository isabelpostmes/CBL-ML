...
fnamebase = 'data/'
plot_bvalues = pd.read_csv(fnamebase + 'bvalues.csv', usecols=range(1,40))
plot_bgvalues = pd.read_csv(fnamebase + 'bgvalues.csv', usecols=range(1,40))
...
for i in range(2):
    ...
    if i == 0:
        ...
        ax.set_xlabel('$\Delta$E1 (eV)', fontsize=20)
    ...
    if i == 1:
        ax.set_ylim([0, 3])
        ax.plot(dE1_array, smooth(bg_bandgaps[i], wl), '--', color='steelblue', label=label1)
        ax.plot(dE1_array, smooth(bg_high[i], wl), '-', color='steelblue', label = label2)
        ax.plot(dE1_array, smooth(bg_low[i], wl), '-', color='steelblue', label = label2)
        ax.fill_between(dE1_array, smooth(bg_high[i], wl), smooth(bg_low[i], wl), \
                    color='steelblue', alpha=.2, label=label2)
        ax.set_yticks([0, .5, 1, 1.5, 2])
        ax.set_ylabel('$E_{bg}$', fontsize=20)
        ax.set_xlabel('$\Delta$E1 (eV)', fontsize=20)
    
    handles, labels = ax.get_legend_handles_labels()
    ax.legend((handles[0], (handles[1], handles[2], handles[3])), (labels[0], labels[1]), \
                  loc = 'upper left', fontsize=14)
    ax.set_title('Spectrum 14', fontsize=15)
...
plt.tight_layout()
plt.savefig('Sability_plots.pdf')
