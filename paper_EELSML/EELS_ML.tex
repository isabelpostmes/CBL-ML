...
the contributions associated to the inelastic scatterings off the specimen.
%
...
%
This novel method 
is used to disentangle the ZLP and
specimen contributions in EEL spectra acquired
in WS$_2$ nanostructures characterised by a flower-like morphology
and a 2H/3R mixed polytypism.
%
From the resulting ZLP-subtracted spectra, we
determine the type and value of the bandgap of polytypic bulk W$_2$,
finding  $E_{\rm BG} = 1.6_{-0.2}^{+0.3}\,{\rm eV}$ with a clear preference for
an indirect bandgap.
%
Further, we demonstrate how it is possible to robustly identify excitonic
transitions down to very small energy losses.
%
Our approach has been implemented in an
open source {\sc Python} package, dubbed {\tt EELSfitter} and based
on {\tt TensorFlow} for the neural network training.
