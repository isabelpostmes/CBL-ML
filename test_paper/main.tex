...
ZLP in vacuum, where we construct a N-dimensional model using NNs. Here you show that you can add many extra variables and use them for prediction and interpolation succesfully
\section{The data set (I)}
The data sets used in this part of the study are listed in table \ref{table:vacuum}.

\begin{table}[H]
\begin{tabular}{|l|l|l|l|l|l|l|l|l|}
\hline
Set & $t_{exp}$ {[}ms{]} & $E_{beam}$ {[}keV{]} & $N_{files}$ & $N_{dat} / file$ & $dE_{min}$ {[}eV{]} & $dE_{max}$ {[}eV{]} & $I_{max}$ & FWHM {[}eV{]} \\ \hline
1        & 100                 & 200                  & 15          & 2048               & -0.96              & +8.51               & 739770       & 0.025         \\
2        & 100                 & 60                   & 7           & 2048               & -0.54              & +5.59               & 326483       & 0.022         \\
3        & 10                  & 200                  & 12          & 2048               & -0.37              & +2.97               & 70913        & 0.003         \\
4        & 10                  & 60                   & 6           & 2048               & -0.40              & +4.78               & 30793        & 0.017         \\ \hline
\end{tabular}

\caption{The data sets of the in vacuum recorded zero loss peaks used for this part of the analysis. We show the number of data points ($N_{files}\times N_{dat}$/file), the energetic range, maximum recorded intensity and FWHM of each set of data.}
\label{table:vacuum}
\end{table}
...
Underline the way to go before diving into detail. Highlight the differences with the pure ZLP signal.
...
\subsection{Neural networks and general strategy}
