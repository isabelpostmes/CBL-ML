...
where $N_{ZLP}$ is the integrated intensity of $I_{ZLP}$.
...
\begin{itemize}
    ...
    \item As 
    \begin{equation}
        \operatorname{Re}\left[\frac{1}{\varepsilon(E)}\right] = \mathcal{C}\{1/\varepsilon(t) - \delta(t)\} = \mathcal{F}\{p(t)\},
    \end{equation}
    and 
    \begin{equation}
        \operatorname{Im}\left[\frac{-1}{\varepsilon(E)}\right] = \mathcal{S}\{1/\varepsilon(t) - \delta(t)\} = \frac{1}{i}\mathcal{F}\{q(t)\},
    \end{equation}
    where $\mathcal{C}$ and $\mathcal{S}$ are the cosine and sine Fourier transforms respectivily, 
\end{itemize}
