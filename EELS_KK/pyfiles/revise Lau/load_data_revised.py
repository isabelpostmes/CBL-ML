...
def load_spectra(path, rg):
    ...
    for filename in os.listdir(path):
        ...
        if filename.endswith(".txt"):
            ...
            df_spectra = df_spectra.append({'x': spectrum, 'y': dE}, ignore_index = True)
...
def load_data(path_vacuum, path_sample, range_vacuum, range_sample = False, pr = False):
    ...
    OUTPUT:
        df_vacuum: pandas data frame with all vacuum spectra
        df_sample: pandas data frame with all sample spectra
    """
...
def shift_norm(df_spectra):
    ...
    for i in df_spectra.index:
        ...
        i_max_y = np.argmax(df_sn.iloc[i].y)
