...
class Spectral_image():
    ...
    def determine_deltaE(self):
        data_avg = np.average(self.data, axis = 0)
        ind_max = np.argmax(data_avg)
        #l = self.data.shape[0]
        deltaE = np.linspace(ind_max * self.ddeltaE, (self.l-ind_max)*self.ddeltaE, self.l)
        return deltaE
    ...
    def calculate_general_ZLPs(self, path_to_models):
        ...
        def make_model(self, inputs, n_outputs):
    ...
    def calc_ZLPs(self, i,j):
        ...
        if not hasattr(self, 'ZLPs_gen'):
            self.calculate_general_ZLPs("iets")
        
        def matching(self, signal):
            ...
            range_1 = self.ZLPs_gen[(self.deltaE < self.dE1) & (self.deltaE >= self.dE0)] * factor_NN + signal[(self.deltaE < self.dE1) & (self.deltaE >= self.dE0)] * factor_dm
            range_2 = self.ZLPs_gen[(self.deltaE >= self.dE1) & (self.deltaE < 3 * self.dE2)]
            range_3 = self.ZLPs_gen[(self.deltaE >= 3 * self.dE2)] * 0
        ...
        for k in range(count): 
            ZLPs[k,:] = self.matching(signal)#matching(energies, np.exp(mean_k), data)
    ...
    def im_dielectric_function(self):
        #TODO
        ...
        for i in range(self.image_shape[0]):
            for j in range(self.image_shape[1]):
                print("calculating dielectric function for pixel " , i,j)
                data_ij = self.get_signal(i,j)[self.deltaE>0]
                ZLPs = self.calc_ZLPs(i,j)
                dielectric_functions = np.zeros(ZLPs.shape)
                for k in range(ZLPs.shape[0]):
                    ZLP_k = ZLPs[k,:]
                    N_ZLP = np.sum(ZLPs)
                    dielectric_functions[k,:] = self.kramers_kronig_hs(data_ij-ZLP_k, N_ZLP = N_ZLP, n =3)
    ...
    def crossings_im(self,  delta = 50):
        self.crossings_E = np.zeros((self.imagesize[0], self.image_shape[1],1))
        self.crossings_n = np.zeros(self.imagesize)
        n_max = 1
        for i in range(self.image_shape[0]):
            print("cross", i)
            for j in range(self.image_shape[1]): 
                #print("cross", i, j)
                crossings_E_ij, n = self.crossings( delta)
                if n > n_max:
    ...
    def crossings(self, delta = 50):
        #l = len(die_fun)
        die_fun_avg = np.zeros(self.l-delta)
        ...
        crossing_E = deltaE_n[crossing]
        n = len(crossing_E)
        return crossing_E, n
            
def load_data(path_to_dmfile):
    dmfile = dm.fileDM('/path/to/area03-eels-SI-aligned.dm4').getDataset(0)
    data = dmfile['data']
    ddeltaE = dmfile['pixelSize'][0]
    pixelSize = dmfile['pixelSize'][1:]
...
def get_prefix(unit, SIunit = None, numeric = True):
    ...
    if prefix == 'T':
        ...
        print("either no or unknown prefix: " + unit + ", asuming no.")
    return 1
